...
const AdminProjects = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.post("http://localhost:5000/api/projects/admin", form);
    setForm({ title: "", description: "", image: "" });
    loadProjects();
  };

  const deleteProject = async (id) => {
    await axios.delete(`http://localhost:5000/api/projects/admin/${id}`);
    loadProjects();
  };

  return (
    <div className="bg-white">
      <h1 className="text-center text-2xl font-bold mb-4"> Admin – My Work Featured Projects</h1>
